...
// ConsoleWeatherForecast SDK retry feature
...
class ConsoleWeatherForecastRetryFeature extends ConsoleWeatherForecastBaseFeature
{
    ...
    public function init(ConsoleWeatherForecastContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ConsoleWeatherForecastContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ConsoleWeatherForecastContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ConsoleWeatherForecastContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
