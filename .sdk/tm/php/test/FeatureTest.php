...
// ConsoleWeatherForecast SDK feature test
//
...
class FtCtrl extends ConsoleWeatherForecastControl
{
...
class FtClient
{
    ...
    private ConsoleWeatherForecastUtility $utility;

    public function __construct(ConsoleWeatherForecastUtility $utility, array $options)
    {
    ...
    public function get_utility(): ConsoleWeatherForecastUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ConsoleWeatherForecastContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ConsoleWeatherForecastContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ConsoleWeatherForecastContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ConsoleWeatherForecastConfig::make_feature($name);
    ...
    private static function buildUrl(ConsoleWeatherForecastSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ConsoleWeatherForecastError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ConsoleWeatherForecastResponse($fetched);
        }
    ...
    private function fail(ConsoleWeatherForecastContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ConsoleWeatherForecastContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
