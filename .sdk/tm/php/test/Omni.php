...
//    only - a typed ConsoleWeatherForecastContext there would make every ctx
...
final class ConsoleWeatherForecastOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ConsoleWeatherForecastContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ConsoleWeatherForecastSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ConsoleWeatherForecastResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ConsoleWeatherForecastError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ConsoleWeatherForecastResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ConsoleWeatherForecastError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ConsoleWeatherForecastError(is_string($code) ? $code : '', $msg);
    }
