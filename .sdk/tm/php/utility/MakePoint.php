...
class ConsoleWeatherForecastMakePoint
{
    public static function call(ConsoleWeatherForecastContext $ctx): array
    {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = ConsoleWeatherForecastHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = ConsoleWeatherForecastHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
