...
from projectname_sdk.core.control import ConsoleWeatherForecastControl
...
class ConsoleWeatherForecastContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = ConsoleWeatherForecastControl()
        ...
        if isinstance(sp, ConsoleWeatherForecastSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ConsoleWeatherForecastResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ConsoleWeatherForecastResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return ConsoleWeatherForecastOperation({})
        ...
        op = ConsoleWeatherForecastOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return ConsoleWeatherForecastError(code, msg, self)
