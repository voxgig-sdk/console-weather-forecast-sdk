# console-weather-forecast-cli

boru-driven command-line client **and** interactive REPL for the ConsoleWeatherForecast
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/console-weather-forecast-cli)
make build

# 2. See usage (words, entities, env vars)
./console-weather-forecast-cli --help

# 3. Provide credentials once, via the environment
export CONSOLE_WEATHER_FORECAST_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./console-weather-forecast-cli load 1 get_current_location_weather            # {id:1} shorthand
./console-weather-forecast-cli load '{id:1}' get_current_location_weather       # explicit match map

# 5. Override the API base URL for a single call
CONSOLE_WEATHER_FORECAST_BASE=https://api.example.com ./console-weather-forecast-cli load 1 get_current_location_weather

# 6. No arguments -> interactive REPL
./console-weather-forecast-cli
console-weather-forecast> load 1 get_current_location_weather
console-weather-forecast> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/console-weather-forecast-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/console-weather-forecast-cli load 1 get_current_location_weather
   ```
...
```sh
./console-weather-forecast-cli load 1 get_current_location_weather          # scalar shorthand for {id:1}
./console-weather-forecast-cli load '{id:1}' get_current_location_weather     # explicit match map
```
...
./console-weather-forecast-cli load 1 get_current_location_weather
```
...
Run with no arguments to open a REPL (prompt `console-weather-forecast>`). Each line is
evaluated as its own boru expression:

```text
$ ./console-weather-forecast-cli
console-weather-forecast> load 1 get_current_location_weather
console-weather-forecast> /help
console-weather-forecast> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_current_location_weather`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/console-weather-forecast-cli`. |
...
(`./console-weather-forecast-cli <expr>`) and interactively (the REPL), and expressions compose the
