// Command ConsoleWeatherForecast-cli is a CLI + REPL for the ConsoleWeatherForecast SDK,
...
const prompt = "console-weather-forecast"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_current_location_weather get_location_weather help location"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CONSOLE_WEATHER_FORECAST_APIKEY (api key), CONSOLE_WEATHER_FORECAST_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CONSOLE_WEATHER_FORECAST_APIKEY carries the API key and
	// CONSOLE_WEATHER_FORECAST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CONSOLE_WEATHER_FORECAST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CONSOLE_WEATHER_FORECAST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewConsoleWeatherForecastSDK(opts)
