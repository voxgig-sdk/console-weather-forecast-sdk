...
func TestGetCurrentLocationWeatherEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		getCurrentLocationWeatherRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_current_location_weather")))
...
func get_current_location_weatherBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CONSOLE_WEATHER_FORECAST_TEST_GET_CURRENT_LOCATION_WEATHER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CONSOLE_WEATHER_FORECAST_TEST_GET_CURRENT_LOCATION_WEATHER_ENTID"])
