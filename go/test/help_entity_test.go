...
func TestHelpEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CONSOLEWEATHERFORECAST_TEST_HELP_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func helpBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CONSOLEWEATHERFORECAST_TEST_HELP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CONSOLEWEATHERFORECAST_TEST_HELP_ENTID"])
