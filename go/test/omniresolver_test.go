...
import (
	...
	omni "github.com/voxgig-sdk/console-weather-forecast-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ConsoleWeatherForecastError); is {
				out := map[string]any{
					"name":    "ConsoleWeatherForecastError",
...
func omniCtx(arg any, client *sdk.ConsoleWeatherForecastSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ConsoleWeatherForecastSDK); is {
			client = live
