...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CONSOLE_WEATHER_FORECAST_TEST_LIVE") == "TRUE" ||
		os.Getenv("CONSOLE_WEATHER_FORECAST_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CONSOLE_WEATHER_FORECAST_TEST_EXPLAIN"); explain != "" {
