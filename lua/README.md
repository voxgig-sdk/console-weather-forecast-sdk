# ConsoleWeatherForecast Lua SDK



The Lua SDK for the ConsoleWeatherForecast API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:GetCurrentLocationWeather()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local get_current_location_weather, err = client:GetCurrentLocationWeather():load()
    if err then error(err) end
...
```lua
local help, err = client:Help():load()
```
...
```
lua/
├── console-weather-forecast_sdk.lua    -- Main SDK module
...
The main module (`console-weather-forecast_sdk`) exports the SDK constructor
