...
The Lua SDK for the ConsoleWeatherForecast API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetCurrentLocationWeather(nil):load({ id = "example_id" }, nil)
...
local result, err = client:ConsoleWeatherForecast(nil):load(
  { id = "test01" }, nil
)
