...
require("console-weather-forecast_types")
...
local ConsoleWeatherForecastSDK = {}
ConsoleWeatherForecastSDK.__index = ConsoleWeatherForecastSDK
...
ConsoleWeatherForecastSDK._make_feature = _make_feature


function ConsoleWeatherForecastSDK.new(options)
  local self = setmetatable({}, ConsoleWeatherForecastSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ConsoleWeatherForecastSDK:options_map()
...
function ConsoleWeatherForecastSDK:get_utility()
  return Utility.copy(self._utility)
end


function ConsoleWeatherForecastSDK:get_root_ctx()
  return self._rootctx
end


function ConsoleWeatherForecastSDK:prepare(fetchargs)
...
function ConsoleWeatherForecastSDK:direct(fetchargs)
...
function ConsoleWeatherForecastSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ConsoleWeatherForecastSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ConsoleWeatherForecastSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ConsoleWeatherForecastSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ConsoleWeatherForecastSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ConsoleWeatherForecastSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:GetCurrentLocationWeather():list() / client:GetCurrentLocationWeather():load({ id = ... })
...
-- Idiomatic facade: client:GetLocationWeather():list() / client:GetLocationWeather():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function ConsoleWeatherForecastSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ConsoleWeatherForecastSDK.new(sdkopts)
  sdk.mode = "test"
...
return ConsoleWeatherForecastSDK
