...
local HelpEntity = {}
HelpEntity.__index = HelpEntity


function HelpEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HelpEntity)
  self._name = "help"
  self._client = client
...
function HelpEntity:get_name()
  return self._name
end


function HelpEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HelpEntity.new(self._client, opts)
end
...
function HelpEntity:mark_deleted()
  self._deleted = true
end


function HelpEntity:deleted()
  return true == self._deleted
end


function HelpEntity:data_set(args)
  if args ~= nil then
...
function HelpEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HelpEntity:match_set(args)
  if args ~= nil then
...
function HelpEntity:stream(action, args, callopts)
...
function HelpEntity:_run_op(ctx, post_done)
  local utility = self._utility
