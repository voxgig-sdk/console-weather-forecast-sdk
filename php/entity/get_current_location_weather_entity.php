...
// ConsoleWeatherForecast SDK GetCurrentLocationWeather entity
...
class GetCurrentLocationWeatherEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "get_current_location_weather";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GetCurrentLocationWeatherEntity($this->_client, $opts);
    }

    /**
     * @param GetCurrentLocationWeather|array $args GetCurrentLocationWeather data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ConsoleWeatherForecastHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetCurrentLocationWeather|array The current GetCurrentLocationWeather data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetCurrentLocationWeather fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ConsoleWeatherForecastHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetCurrentLocationWeather fields).
     */
    ...
    /**
     * Load a single GetCurrentLocationWeather.
     *
     * @param GetCurrentLocationWeatherLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetCurrentLocationWeatherLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetCurrentLocationWeather|array The loaded GetCurrentLocationWeather as an assoc-array at the
     *   SDK boundary; throws ConsoleWeatherForecastError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ConsoleWeatherForecastHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
