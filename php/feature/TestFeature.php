...
// ConsoleWeatherForecast SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ConsoleWeatherForecastTestFeature extends ConsoleWeatherForecastBaseFeature
{
    ...
    public function init(ConsoleWeatherForecastContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ConsoleWeatherForecastContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ConsoleWeatherForecastParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ConsoleWeatherForecastContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ConsoleWeatherForecastContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ConsoleWeatherForecastParam::call($ctx, $k);
