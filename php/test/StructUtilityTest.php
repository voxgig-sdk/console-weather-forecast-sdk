...
// ConsoleWeatherForecast SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ConsoleWeatherForecastOmni::makeStructRunner(
    ...
    {
        return ConsoleWeatherForecastOmni::objmodel(ConsoleWeatherForecastOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return ConsoleWeatherForecastOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return ConsoleWeatherForecastOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ConsoleWeatherForecastOmni::structfix($now, Struct::undef()))
                        !== json_encode(ConsoleWeatherForecastOmni::structfix($val, Struct::undef()))) {
