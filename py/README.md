...
The Python SDK for the ConsoleWeatherForecast API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GetCurrentLocationWeather(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ConsoleWeatherForecast(None).load(
    {"id": "test01"}, None
)
