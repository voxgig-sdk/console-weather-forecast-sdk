...
class GetLocationWeatherEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetLocationWeatherEntity(self._client, opts)
    ...
    def data_get(self) -> GetLocationWeather:
    ...
    def match_get(self) -> GetLocationWeather:
    ...
    def load(self, reqmatch=None, ctrl=None) -> GetLocationWeather:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GetLocationWeather().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
