# ConsoleWeatherForecast SDK test runner
...
class ConsoleWeatherForecastTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ConsoleWeatherForecastTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ConsoleWeatherForecastTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ConsoleWeatherForecastTestRunner.getenv("CONSOLE_WEATHER_FORECAST_TEST_LIVE")
        override = ConsoleWeatherForecastTestRunner.getenv("CONSOLE_WEATHER_FORECAST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ConsoleWeatherForecastTestRunner.getenv(key)
        ...
        explain = ConsoleWeatherForecastTestRunner.getenv("CONSOLE_WEATHER_FORECAST_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ConsoleWeatherForecastTestRunner._test_control is not None:
            return ConsoleWeatherForecastTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ConsoleWeatherForecastTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ConsoleWeatherForecastTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ConsoleWeatherForecastTestRunner.load_test_control()
...
def load_env_local():
    ConsoleWeatherForecastTestRunner.load_env_local()


def env_override(m):
    return ConsoleWeatherForecastTestRunner.env_override(m)


def entity_data(v):
    return ConsoleWeatherForecastTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ConsoleWeatherForecastTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ConsoleWeatherForecastTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ConsoleWeatherForecastTestRunner.load_test_control()


def live_client_options():
    return ConsoleWeatherForecastTestRunner.live_client_options()


def live_delay_ms():
    return ConsoleWeatherForecastTestRunner.live_delay_ms()
