...
class TestHelpDirect:

    def test_should_direct_load_help(self):
        setup = _help_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-help", "live" if setup["live"] else "unit")
        if _skip:
...
def _help_direct_setup(mockres):
