# ConsoleWeatherForecast Ruby SDK



The Ruby SDK for the ConsoleWeatherForecast API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.GetCurrentLocationWeather` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
require_relative "ConsoleWeatherForecast_sdk"
client = ConsoleWeatherForecastSDK.new(options)
```
...
```ruby
client = ConsoleWeatherForecastSDK.test(testopts, sdkopts)
```
...
| `GetCurrentLocationWeather` | `(data) -> GetCurrentLocationWeatherEntity` | Create a GetCurrentLocationWeather entity instance. |
| `GetLocationWeather` | `(data) -> GetLocationWeatherEntity` | Create a GetLocationWeather entity instance. |
...
raise a `ConsoleWeatherForecastError` (a `StandardError` subclass), so wrap
...
```
rb/
├── ConsoleWeatherForecast_sdk.rb       -- Main SDK module
...
The main module (`ConsoleWeatherForecast_sdk`) exports the SDK class
