...
The Ruby SDK for the ConsoleWeatherForecast API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.GetCurrentLocationWeather(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.ConsoleWeatherForecast(nil).load(
  { "id" => "test01" }, nil
)
