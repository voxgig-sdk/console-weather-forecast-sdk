# ConsoleWeatherForecast Ruby SDK Reference

Complete API reference for the ConsoleWeatherForecast Ruby SDK.
...
```ruby
require_relative 'console-weather-forecast_sdk'
...
#### `ConsoleWeatherForecastSDK.test(testopts = nil, sdkopts = nil)`
...
```ruby
result, err = client.GetCurrentLocationWeather.load({ "id" => "get_current_location_weather_id" })
```
...
```ruby
result, err = client.GetLocationWeather.load({ "id" => "get_location_weather_id" })
```
...
```ruby
result, err = client.Help.load({ "id" => "help_id" })
```
