...
class HelpDirectTest < Minitest::Test
  def test_direct_load_help
    setup = help_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-help", setup[:live] ? "live" : "unit")
    if _should_skip
