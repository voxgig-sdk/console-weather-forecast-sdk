...
module ConsoleWeatherForecastUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ConsoleWeatherForecastResponse.new({})
    ...
    if fetch_err
      ...
      response = ConsoleWeatherForecastResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ConsoleWeatherForecastResponse.new(fetched)
    else
