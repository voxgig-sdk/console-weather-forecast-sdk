# ConsoleWeatherForecast TypeScript SDK



The TypeScript SDK for the ConsoleWeatherForecast API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.GetCurrentLocationWeather()` — each with a small set of operations (`load`)
...
```ts
try {
  const location = await client.Location().load({ location: "example" })
...
await entity.load({ location: 'example_location' })
...
| `tester(testopts?, sdkopts?)` | `ConsoleWeatherForecastSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `ConsoleWeatherForecastSDK.test(testopts?, sdkopts?)` | `ConsoleWeatherForecastSDK` | Create a test-mode client. |
...
```ts
const get_current_location_weather = await client.GetCurrentLocationWeather().load()
```
...
```ts
const help = await client.Help().load()
```
...
├── src/
│   ├── ConsoleWeatherForecastSDK.ts        # Main SDK class
...
await location.load({ location: "example" })
