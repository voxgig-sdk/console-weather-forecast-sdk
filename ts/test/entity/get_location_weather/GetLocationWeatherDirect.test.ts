...
import { ConsoleWeatherForecastSDK } from '../../..'
...
describe('GetLocationWeatherDirect', async () => {
  ...
  test('direct-load-get_location_weather', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_location_weather', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CONSOLE_WEATHER_FORECAST_TEST_GET_LOCATION_WEATHER_ENTID']
