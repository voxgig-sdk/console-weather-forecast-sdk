...
//     (\`new ConsoleWeatherForecastSDK(...)\` / \`ConsoleWeatherForecastSDK.test(...)\` become
//     \`ConsoleWeatherForecastSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ConsoleWeatherForecastSDK'
...
const TEST_SEED = {"entity":{"get_current_location_weather":{"test01":{"id":"test01"}},"get_location_weather":{"test01":{"id":"test01"}},"help":{"test01":{"id":"test01"}},"location":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ConsoleWeatherForecastSDK(...)\` and any
// \`ConsoleWeatherForecastSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ConsoleWeatherForecastSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ConsoleWeatherForecastSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
